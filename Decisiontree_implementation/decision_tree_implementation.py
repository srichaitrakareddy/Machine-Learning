...
def mutual_information(x, y):
    ...
    for key, Value in X.items():
        temporary = []
        for i in Value:
            temporary.append(y[i])
        entropy_Y_X = entropy(temporary)
        Prob_X = x.count(key)/len(x)
        conditional_entropy += entropy_Y_X*Prob_X

    info_gain = entropy_of_Y - conditional_entropy
...
def id3(x, y, attribute_value_pairs=None, depth=0, max_depth=5):
    ...
    * Leaf nodes are (majority) class labels
    ...
    for each in attribute_value_pairs:
        ...
        info_gain = mutual_information(temp, y)
    ...
    set_1 = attribute_value_pairs.copy()
    set_2 = attribute_value_pairs.copy()

    set_1.remove(pair)
    set_2.remove(pair)
    #recursive call
    root ={(pair[0], pair[1], True): id3(X_True, Y_True, set_1, depth+1, max_depth),             (pair[0], pair[1], False): id3(X_False, Y_False, set_2, depth+1, max_depth)}
...
def compute_error(y_true, y_pred):
    ...
    # INSERT YOUR CODE HERE
    #We are calculating the error by keeping a track of when the true value and the predicted value are different
     c = 0 
